...
def task(cmd, jobnum=False, timeout=None, run=True):
    print(cmd)
    if run:
        result = subprocess.run(cmd, timeout=timeout, capture_output=True, encoding='utf-8', shell=True)
        print(result.stdout)
        if jobnum:
...
class RenderJob:
    ...
    def __init__(self, imagecmd, ntrain, nval, ntest, nbatch=10, imagetime=10, posttime=300, filetag='render', sleeptime=30, email='', precmd=''):
        print("initializing render job")
    ...
    def render(self, batchlist=None):
        ...
        for k in batchiter:
            ...
            batch = RenderBatch(imagecmd=self.imagecmd, low=i1, high=i2, batchfile=batchfile, imagetime=self.imagetime, posttime=self.posttime, emailopt=self.emailopt, precmd=self.precmd)
            batch.start()
...
class RenderBatch:
    ...
    def __init__(self, imagecmd, low, high, batchfile, imagetime=10, posttime=300, emailopt='', precmd=''):
        print("initializing render batch")
        self.imagecmd = imagecmd
        self.low = low
        self.high = high
    ...
    def start(self):
        ...
        sbatchcmd = f'sbatch --time={slurm.sec2time(self.totaltime)} {self.emailopt} $SLURM_LIBDIR/job_sbatch.sh \'{self.precmd}\' \'{rendercmd}\''
...
def batch(imagecmd, imagedir, low, high, batchfile, imagetime=10, rendertime=np.inf):
    ...
    for i in range(low, high+1):
        ...
        filename = os.path.join(imagedir, f'image{i:06d}')
