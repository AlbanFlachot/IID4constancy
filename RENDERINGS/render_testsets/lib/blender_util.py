...
class Intrinsic:
    ...
    def reflectance(self):
        return bpy.context.scene.view_layers[VL].use_pass_diffuse_color

    @reflectance.setter
    def reflectance(self,b):

        Intrinsic.delnodes( [ 'RefAlpha', 'RefFile' ] )
        bpy.context.scene.view_layers[VL].use_pass_diffuse_color = b
    ...
    @property
    def normals(self):
        return bpy.context.scene.view_layers[VL].use_pass_normal

    @normals.setter
    def normals(self, b):

        Intrinsic.delnodes( [ 'NrmScale', 'NrmBias', 'NrmFile' ] )
        bpy.context.scene.view_layers[VL].use_pass_normal = b
    ...
    @property
    def depth(self):
        return bpy.context.scene.view_layers[VL].use_pass_z

    @depth.setter
    def depth(self, b):

        Intrinsic.delnodes( [ 'DepthFile' ] )
        bpy.context.scene.view_layers[VL].use_pass_z = b
    ...
    @property
    def objectid(self):
        return bpy.context.scene.view_layers[VL].use_pass_object_index

    @objectid.setter
    def objectid(self, b):

        Intrinsic.delnodes( [ 'ObjectIDFile' ] )
        bpy.context.scene.view_layers[VL].use_pass_object_index = b
...
def patterns( obj=None,name='image' , tex2apply = 'None', difflum = 1):
    ...
    if tex2apply == 'None':
        ...
        #img_base = np.ones((1000, 1000, 3))
        #rgb = np.array([difflum/10, difflum/10, difflum/10])
        #img_base *= rgb
        ...
        outputImg = bpy.data.images.new("pattern.png", 1000, 1000, alpha=True)
        outputImg.pixels = (np.asarray(patt.pixels))**(1/2.2)
