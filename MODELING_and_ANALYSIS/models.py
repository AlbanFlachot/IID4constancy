...
class encoder0(nn.Module ):
    def __init__(self, cascadeLevel = 0,  nb_channels = 3, isSeg = False,):
        ...
        if self.cascadeLevel == 0:
            self.conv1 = nn.Conv2d(in_channels=self.nb_channels, out_channels=64, kernel_size=4, stride=2, bias =True )
        else:
...
class decoder0(nn.Module ):
    def __init__(self, mode=0, nb_channels = 3):
        ...
        self.dconvFinal = nn.Conv2d(in_channels=64, out_channels=self.nb_channels, kernel_size = 3, stride=1, bias=True)
    ...
    def forward(self, im, x1, x2, x3, x4, x5, x6 ):
        ...
        if self.mode == 0:
            ...
            x_out = torch.clamp(1.01 * torch.tanh(x_orig ), -1, 1)
        elif self.mode == 5: # special for unsupervised learning
            x_orig = 1.01 * torch.tanh(x_orig)
            x_origmean = torch.mean(x_orig)
            x_orig = (x_orig - x_origmean)/torch.std(x_orig) + x_origmean
            x_out = torch.clamp(x_orig, -1, 1)
        elif self.mode == 7: # mode to include bn at the end for albedo
            x_orig = self.dbn1(x_orig)
            x_out = torch.clamp(1.01 * torch.tanh(x_orig), -1, 1)
        return x_out
