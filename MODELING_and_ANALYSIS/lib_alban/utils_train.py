...
def compute_smooth_loss(smooth, input, pred):
    scale_factor = 1
    pred_weighed = smooth(
        input, pred, patch_size=3, alpha=10, scale_factor=scale_factor
    )
    smoothloss = (
        mse_loss(pred,
                 pred_weighed)
    )
    return pred_weighed, smoothloss
...
class LiNet:
    ...
    def train(self, epoch, writer):
        ...
        for i, dataBatch in enumerate(brdfLoader):
            ...
            # Initial Prediction
            x1, x2, x3, x4, x5, x6 = self.encoder(imBatch)
...
class AlbanNet_supervised:
    ...
    def __init__(self, dataset, experiment, batchsize=2, imWidth0=256, imHeight0=256, cuda=True, gpuID=[0],
                 nb_channels=1, smoothing=True, lrStep=10, noRend=False, noise=False):
        self.rootdir = 'LiNet/'
        self.imWidth = imWidth0
        self.imHeight = imHeight0
    ...
    def train(self, nbepoch, writer):
        ...
        for epoch in range(nbepoch):
            ...
            brdfDataset = dataLoader.BatchLoader_AlbanNet(self.dataRoot, imWidth=256, imHeight=256,
            ...
            for i, dataBatch in enumerate(brdfLoader):
                ...
                # Load the image from cpu to gpu
                im_cpu_noiseless = (dataBatch['im'])
                imBatch_noiseless = Variable(im_cpu_noiseless).cuda()
                if self.noise:
                    im_cpu = (dataBatch['noisy_im'])
                    imBatch = Variable(im_cpu).cuda()
                else:
                    imBatch = imBatch_noiseless
                ...
                albedoPred = 0.5 * (self.albedoDecoder(imBatch, x1, x2, x3, x4, x5, x6) + 1)
                # normalPred = self.normalDecoder(imBatch, x1, x2, x3, x4, x5, x6)
                illuPred = (self.illuDecoder(imBatch, x1, x2, x3, x4, x5, x6) + 1)  # values up to 2 allowed
                ...
                if self.noRend:
                    rendErrs = torch.tensor([0]).cuda(self.gpuID)
                else:
                    rendErrs = torch.mean(torch.square(rendererPred - imBatch_noiseless))
                ...
                if self.j % 500 == 0:
                    writer.add_image('Input', vutils.make_grid((imBatch) ** (1 / 2.2), normalize=True,
                                                               scale_each=True), self.j)
                    # writer.add_image('Pred Input', vutils.make_grid((albedoPreds[0]*illuPreds[0])**(1/2.2)), self.j)
                    writer.add_image('Input Pred', vutils.make_grid(rendererPred ** (1 / 2.2), normalize=True,
                                                                    scale_each=True), self.j)
                    writer.add_image('Albedos Ref', vutils.make_grid(albedoBatch ** (1 / 2.2), normalize=True,
                                                                     scale_each=True), self.j)
                    writer.add_image('Albedos Pred', vutils.make_grid(albedoPred ** (1 / 2.2), normalize=True,
                                                                      scale_each=True), self.j)
                    ...
                    writer.add_image('Illus Ref', vutils.make_grid(illuBatch ** (1 / 2.2), normalize=True,
                                                                   scale_each=True), self.j)
                    writer.add_image('Illus Pred', vutils.make_grid(illuPred ** (1 / 2.2), normalize=True,
                                                                    scale_each=True), self.j)
                    writer.add_image('Input Rend', vutils.make_grid(illuPred ** (1 / 2.2), normalize=True,
                                                                    scale_each=True), self.j)
...
class AlbanNet_unsupervised:
    ...
    def __init__(self, dataset, experiment, batchsize=1, imWidth0=256, imHeight0=256, cuda=True, gpuID=[0],
                 nb_channels=1, smoothing=True, lrStep=10):
        self.rootdir = 'LiNet/'
        self.imWidth = imWidth0
        self.imHeight = imHeight0
        ...
        if self.nb_channels == 1:
            ...
            self.smooth = smoothing_lib.WeightedAverage_color()
    ...
    def train(self, nbepoch, writer, contraW=1000, contraLoss='triplet'):
        triW = contraW
        trivialW_start = contraW / 2  # loss to avoid trivial solutions at the start
        rendW = 10
        smoothW = contraW
        ...
        for epoch in range(nbepoch):
            trivialW = trivialW_start / (epoch + 1)  # progressively discount the constraint
            ...
            for i, dataBatch in enumerate(brdfLoader):
                ...
                isgrayBatch = np.squeeze(dataBatch['isgray'], -1)
                ...
                illuPred1 = (self.illuDecoder(im1Batch, x11, x21, x31, x41, x51, x61) + 1)
                ...
                illuPred2 = (self.illuDecoder(im2Batch, x12, x22, x32, x42, x52, x62) + 1)
                ...
                intrinsicPreds1 = torch.stack((albedoPred1, illuPred1),
                                              dim=1)  # ignore normals for now, normalize both intrinsic to make them more comparable
                ...
                im1Batch = torch.clamp(im1Batch, 0, 1)
                im2Batch = torch.clamp(im2Batch, 0, 1)
                ## Rendering error
                ...
                ## Trivial loss
                TrivLoss = torch.var(
                    torch.mean(albedoPred1, axis=(0, 2, 3)) + torch.mean(albedoPred2, axis=(0, 2, 3))) / 2

                ## Achromatic loss
                achrom_alb = 0.5 * torch.mean(torch.square(
                    albedoPred1[isgrayBatch] - albedoPred1[isgrayBatch].mean(1, keepdim=True))) + 0.5 * torch.mean(
                    torch.square(albedoPred2[isgrayBatch] - albedoPred2[isgrayBatch].mean(1, keepdim=True)))
                achrom_illu = 0.5 * torch.mean(torch.square(
                    illuPred1[isgrayBatch] - illuPred1[isgrayBatch].mean(1, keepdim=True))) + 0.5 * torch.mean(
                    torch.square(illuPred2[isgrayBatch] - illuPred2[isgrayBatch].mean(1, keepdim=True)))
                AchromLoss = 0.5 * achrom_alb + 0.5 * achrom_illu

                # Smoothness losses
                smoothErrs = torch.tensor([0])
                if self.smoothing:
                    smoothed_Alb1, smoothErrAlb1 = compute_smooth_loss(self.smooth, im1Batch, albedoPred1)
                    smoothed_Alb2, smoothErrAlb2 = compute_smooth_loss(self.smooth, im2Batch, albedoPred2)

                    smoothErrs = 2 * smoothErrAlb1 + 2 * smoothErrAlb2  # + 2 * (
                    # smoothErrIllu1 + smoothErrIllu2) + 2 * smoothErrRend1 + 2 * smoothErrRend2
                    # Back propagate the gradients

                    totalErr = 4 * triW * tripletErrs + rendW * rendErrs + smoothW * smoothErrs + trivialW * TrivLoss + triW * AchromLoss
                else:
                    totalErr = 4 * triW * tripletErrs + rendW * rendErrs + trivialW * TrivLoss + triW * AchromLoss
                totalErr.backward()

                if self.j % 10 == 0:
                    writer.add_scalar('Err Triplet', triW * tripletErrs, self.j)
                    writer.add_scalar('Err Rend', rendW * rendErrs, self.j)
                    writer.add_scalar('Err smooth', smoothW * smoothErrs, self.j)
                    writer.add_scalar('Err Tot', totalErr, self.j)
                    writer.add_scalar('Err Achrom', triW * AchromLoss, self.j)
                if self.j % 1000 == 0:
                    im2Batch = torch.clamp(im2Batch, 0, 1)
                    writer.add_image('Input1', vutils.make_grid((im1Batch) ** (1 / 2.2), normalize=True,
                                                                scale_each=True), self.j)
                    writer.add_image('Input2', vutils.make_grid((im2Batch) ** (1 / 2.2), normalize=True,
                                                                scale_each=True), self.j)
                    # writer.add_image('Pred Input', vutils.make_grid((albedoPreds[0]*illuPreds[0])**(1/2.2)), self.j)
                    writer.add_image('Input1 Pred ', vutils.make_grid(rendererPred1 ** (1 / 2.2), normalize=True,
                                                                      scale_each=True), self.j)
                    writer.add_image('Input2 Pred ', vutils.make_grid(rendererPred2 ** (1 / 2.2), normalize=True,
                                                                      scale_each=True), self.j)
                    # writer.add_image('Albedos Ref', vutils.make_grid(albedoBatch1**(1/2.2)), self.j)
                    writer.add_image('Albedos1 Pred ',
                                     vutils.make_grid(albedoPred1 ** (1 / 2.2), normalize=True,
                                                      scale_each=True), self.j)
                    writer.add_image('Albedos2 Pred ',
                                     vutils.make_grid(albedoPred2 ** (1 / 2.2), normalize=True,
                                                      scale_each=True), self.j)
                    '''writer.add_image('Albedos1 smooth ',
                                     vutils.make_grid(smoothed_Im1 ** (1 / 2.2), normalize=True,
                                                      scale_each=True), self.j)
                    writer.add_image('Albedos2 smooth ',
                                     vutils.make_grid(smoothed_Im2 ** (1 / 2.2), normalize=True,
                                                      scale_each=True), self.j)'''
                    writer.add_image('Illus1 Pred ',
                                     vutils.make_grid(10 * illuPred1[:, :, 10:-10, 10:-10] ** (1 / 2.2), normalize=True,
                                                      scale_each=True), self.j)
                    writer.add_image('Illus2 Pred ',
                                     vutils.make_grid(10 * illuPred2[:, :, 10:-10, 10:-10] ** (1 / 2.2), normalize=True,
                                                      scale_each=True), self.j)
                ...
                # Output training error
                utils.writeErrToScreen('triplet', triW * tripletErrs, epoch, self.j)
                utils.writeErrToScreen('rendering', [rendW * rendErrs], epoch, self.j)
                utils.writeErrToScreen('smoothing', [smoothW * smoothErrs], epoch, self.j)

                utils.writeErrToFile('triplet', tripletErrs, trainingLog, epoch, self.j)
                utils.writeErrToFile('rendering', [rendErrs], trainingLog, epoch, self.j)

                tripletErrsNpList = np.concatenate([tripletErrsNpList, utils.turnErrorIntoNumpy([tripletErrs])], axis=0)
                rendErrsNpList = np.concatenate([rendErrsNpList, utils.turnErrorIntoNumpy([rendErrs])], axis=0)
                if self.j % 1000 == 0:
                    utils.writeImageToFile(((albedoPred1[:, :, 10:-10, 10:-10]) ** (1.0 / 2.2)).data,
                                           '{0}/{1}_albedopred1.png'.format(self.experiment, self.j))
                    utils.writeImageToFile(((albedoPred2[:, :, 10:-10, 10:-10]) ** (1.0 / 2.2)).data,
                                           '{0}/{1}_albedopred2.png'.format(self.experiment, self.j))
                    utils.writeImageToFile(((im1Batch) ** (1.0 / 2.2)).data,
                                           '{0}/{1}_input1.png'.format(self.experiment, self.j))
                    utils.writeImageToFile(((im2Batch + 0.2) ** (1.0 / 2.2)).data,
                                           '{0}/{1}_input2.png'.format(self.experiment, self.j))
                    utils.writeImageToFile(((illuPred1[:, :, 10:-10, 10:-10]) ** (1.0 / 2.2)).data,
                                           '{0}/{1}_illupred1.png'.format(self.experiment, self.j))
                    utils.writeImageToFile(((illuPred2[:, :, 10:-10, 10:-10]) ** (1.0 / 2.2)).data,
                                           '{0}/{1}_illupred2.png'.format(self.experiment, self.j))
                    utils.writeImageToFile(((rendererPred1) ** (1.0 / 2.2)).data,
                                           '{0}/{1}_rendPred1.png'.format(self.experiment, self.j))
                    utils.writeImageToFile(((rendererPred2) ** (1.0 / 2.2)).data,
                                           '{0}/{1}_rendPred2.png'.format(self.experiment, self.j))
